...
async function generatePaginationPages(pageSize) {
    ...
    // Clean up old page_*.md files first
    const existingPageFiles = await globby('page_*.md', { cwd: paths })
    for (const file of existingPageFiles) {
        await fs.remove(path.join(paths, file))
    }
...
async function getPostMDFilePaths() {
    let paths = await globby(['**.md'], {
        ignore: ['node_modules', 'posts/drafts/**', 'README.md']
    })
